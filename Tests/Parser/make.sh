...
echo "
Performing parser/regression tests on Perl4 module...
"
for i in ${IFILE}; do 
necho "    Testing (Perl4) : ${i}.i"; 
if ../../swig -I../../swig_lib -perl4 -dnone -c++ -objc ${i}.i >${i}.perl4.output 2>&1; then 
# Look for a regression testing file
    if test -f ${i}_regr.perl4.c; then
         if diff ${i}_regr.perl4.c ${i}_wrap.c > ${i}.perl4.diff; then
             echo " ....... passed"
             rm -f ${i}.perl4.diff
         else
              echo " ....... FAILED COMPARE (see ${i}.perl4.diff) ***";
              echo "${i}.i (Perl4) ....... FAILED COMPARE (see ${i}.perl4.diff) ***" >> ${LOGFILE}
         fi;
    else
# Regression testing file doesn't exist, create it
         echo " ....... passed (creating ${i}_regr.perl4.c)"
         mv ${i}_wrap.c ${i}_regr.perl4.c
    fi;
rm -f ${i}_wrap*
else 
    echo " ....... FAILED (INTERNAL ERROR) ***";
    echo "${i}.i (Perl4) ....... FAILED (INTERNAL ERROR) ***" >> ${LOGFILE}
fi;
done
